...
<img width="1509" height="733" alt="image" src="https://github.com/user-attachments/assets/1b1c58a5-ea07-4a68-8528-752c5bdc4e77" />
...
Минимальные версии: **UniFi OS 5.1.33** и **UniFi Network 10.6.101**. Более новые версии допускаются; ограничения по названию модели нет — можно пробовать UDM, UDR, UDM Pro, SE и другие шлюзы UniFi.

Текущий пакет содержит движки **ARM64 (aarch64)** и требует системные компоненты UniFi, перечисленные установщиком. На другой архитектуре установка остановится. Другие модели и версии пока не тестировались: разрешение установки не означает подтверждённую совместимость.

Контрольные суммы и пути файлов интерфейса проверяются отдельно. Если новая версия OS/Network меняет эти файлы, установка остановится до применения патча. Это ограничение совместимости патча, а не верхний предел версии. Предупреждение против обновления уже установленной системы остаётся в силе.
...
```sh
curl -fL https://raw.githubusercontent.com/DjGreeZ/AmneziaWG-Unifi-DreamMachine/v0.2.1/install.sh -o /tmp/awg-install.sh && sh /tmp/awg-install.sh
```

Команда скачает пакет фиксированной версии, проверит SHA-256, минимальные версии системы и совместимость файлов интерфейса, затем установит компоненты. Сборка программ на UDM не требуется. Если проверка совместимости не прошла, не обходите её.
...
`AmneziaWG-UniFi-v0.2.1-source.tar.gz` содержит код интеграции, установщик, движки, исходники движков и уведомления о лицензиях. Оригинальные файлы интерфейса UniFi в пакет не включены: изменения применяются локально к установленной системе.
